(() => {
  ...
  function describe(input, init = {}) {
    ...
    if (method === "DELETE" || suffix === "entry") {
      title = method === "DELETE" ? "Подтвердить удаление" : title;
      danger = true;
  ...
  function ensureModal() {
    ...
    modal.innerHTML = `
      <section class="analysis-confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="analysisConfirmTitle">
        <div class="analysis-confirm-head">
          <div>
            <p class="analysis-confirm-kicker">CONTROL CENTER</p>
            <h3 id="analysisConfirmTitle">Подтвердить изменение</h3>
          </div>
        </div>
  ...
  function askConfirmation(input, init) {
    if (pending) return Promise.resolve(false);
    const details = describe(input, init);
    const modal = ensureModal();
  ...
  window.fetch = async function confirmedFetch(input, init = {}) {
    ...
    const accepted = await askConfirmation(input, init);
    if (!accepted) {
      throw new DOMException("Изменение отменено пользователем", "AbortError");
    }
