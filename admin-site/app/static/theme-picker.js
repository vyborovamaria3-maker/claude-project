(() => {
  ...
  function applyTheme(theme) {
    ...
    root.style.setProperty("--theme-soft", theme.bgSoft);
    ...
    const dot = document.querySelector("#themeTriggerDot");
  ...
  function buildPicker() {
    ...
    panel.innerHTML = `<div class="theme-card" role="dialog" aria-modal="true" aria-label="Цветовая тема">
      ...
      ${groups.map(([label, ids]) => `<section class="theme-group"><p class="theme-group-title">${escapeHtml(label)}</p><div class="theme-grid">${ids.map((id) => {
        ...
        return `<button type="button" class="theme-option" data-theme-id="${t.id}"><span class="theme-swatch" style="background:linear-gradient(135deg,${t.bg} 0 54%,${t.accent} 55% 100%)"></span><span><strong>${escapeHtml(t.name)}</strong><small>${t.bg} · ${t.accent}</small></span></button>`;
    ...
    panel.addEventListener("click", (event) => {
      ...
      if (!option) return;
      applyTheme(findTheme(option.dataset.themeId));
    });
  }

  const stored = (() => { try { return localStorage.getItem(STORAGE_KEY); } catch { return null; } })();
  applyTheme(findTheme(stored || "emerald-abyss"));
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", () => { buildPicker(); applyTheme(findTheme(stored || "emerald-abyss")); });
  else { buildPicker(); applyTheme(findTheme(stored || "emerald-abyss")); }
})();