...
export function buildEntryThesis(args: BuildEntryThesisArgs): EntryThesis {
  ...
  } else if ((h1 ?? 0) <= 4 && evidenceSupportScore >= 72 && model.riskScore < 55) {
  ...
  if (priceState === "overheated_vs_signal" && severityRank(action) > severityRank("late_weak")) {
  ...
  if (h1 != null) {
    if (h1 >= 20) alreadyPricedIn.push(`Цена уже выросла на ${pct(h1)} за 1ч — значительная часть краткосрочного импульса может быть уже заложена в цену.`);
    else if (h1 >= 8) alreadyPricedIn.push(`За 1ч цена уже прошла ${pct(h1)}: вход не ранний, но движение ещё не обязательно перегрето.`);
    else if (h1 <= -8) whyNow.push(`Цена откатила на ${pct(h1)} за 1ч; если структура источников сохраняется, это улучшает соотношение цены к подтверждению.`);
    else whyNow.push(`Изменение за 1ч ${pct(h1)} не выглядит экстремальным относительно текущего сигнала.`);
  }
  ...
  let qwenView = qwen?.oneLineVerdict?.trim() || qwenFallbackView(ai);
  ...
  const priceExplanation = priceState === "overheated_vs_signal"
    ...
    : priceState === "stretched_vs_signal"
      ...
      : priceState === "discounted_vs_signal"
        ...
        : priceState === "reasonable_vs_signal"
          ? `Текущая цена выглядит приемлемой относительно силы подтверждений: нет явного вертикального перегрева, но вход всё равно зависит от продолжения cross-source подтверждения.`
  ...
  const thesis = action === "strong_entry"
    ...
    : action === "consider"
      ...
      : action === "wait_confirmation"
        ? `Сейчас лучше ждать подтверждения, а не догонять движение: качество монеты может быть нормальным, но цена/источники ещё не дают достаточно чистого момента входа.`
        : action === "late_weak"
          ...
          : `Текущий входной тезис слабый: риск и/или перегрев перевешивают доступные подтверждения. Пока система не видит достаточного основания считать текущую цену качественным моментом входа.`;
  ...
  return {
    ...
    thesis,
    priceExplanation,
    whyNow: unique(whyNow).slice(0, 7),
    alreadyPricedIn: unique(alreadyPricedIn).slice(0, 6),
    confirmationNeeded: unique(confirmationNeeded).slice(0, 7),
    invalidation: unique(invalidation).slice(0, 7),
    qwenView,
    qwenAgreement,
