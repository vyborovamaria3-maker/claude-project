...
def test_resumable_scan_rolls_back_interrupted_member_and_skips_completed_json(
...
def test_retry_policy_does_not_retry_deterministic_parser_errors() -> None:
    ...
    assert (
        tgdataset_resilient._is_retryable_stream_error(json.JSONDecodeError("bad json", "{", 0))
        is False
    )
