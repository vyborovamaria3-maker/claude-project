...
def event(platform: str, minutes_ago: int, *, source: str, text: str, metrics: dict | None = None):
    return SimpleNamespace(
        platform=platform, source_handle=source, source_name=source, text=text,
        event_type="token_mention", occurred_at=NOW - timedelta(minutes=minutes_ago),
        metrics=metrics or {},
    )
...
def call(hours_ago: int, outcome: str, *, channel_id: int = 1, roi: float = 2.5, window: int = 72):
    return SimpleNamespace(
        called_at=NOW - timedelta(hours=hours_ago), outcome=outcome, is_explicit_call=True,
        call_market_cap_usd=40_000, roi_multiple=roi, channel_id=channel_id,
...
def test_channel_reputation_uses_only_matured_prior_calls() -> None:
    ...
    without = build_historical_features(
        mint=MINT, cutoff=NOW, creation_date=NOW - timedelta(hours=2),
        events=events, prior_calls_by_channel={1: [immature]},
    )
    with_history = build_historical_features(
        mint=MINT, cutoff=NOW, creation_date=NOW - timedelta(hours=2),
        events=events, prior_calls_by_channel={1: [immature, matured]},
    )
...
def test_missing_x_does_not_receive_clean_x_bonus() -> None:
    features = build_historical_features(
        mint=MINT, cutoff=NOW, creation_date=NOW - timedelta(hours=1),
        events=[event("telegram", 10, source="alpha", text="early gem", metrics={"explicit_call": True})],
...
def test_mutable_x_metrics_are_excluded_from_causal_features() -> None:
    common = [
        event("x", 20, source="verified", text="organic research", metrics={"verified": True, "likes": 10}),
        event("x", 10, source="plain", text="different discussion", metrics={"verified": False, "likes": 5}),
    ]
    inflated = [
        event("x", 20, source="verified", text="organic research", metrics={"verified": True, "likes": 9_999_999, "views": 99_999_999}),
        event("x", 10, source="plain", text="different discussion", metrics={"verified": True, "likes": 8_888_888, "views": 88_888_888}),
    ]
    base = build_historical_features(mint=MINT, cutoff=NOW, creation_date=None, events=common, prior_calls_by_channel={})
    future = build_historical_features(mint=MINT, cutoff=NOW, creation_date=None, events=inflated, prior_calls_by_channel={})
...
def test_missing_channel_history_is_not_perfect_safety() -> None:
    ...
    good_history = build_historical_features(
        mint=MINT, cutoff=NOW, creation_date=None, events=events,
...
def test_promotional_copy_paste_raises_risk() -> None:
    ...
    clean_features = build_historical_features(mint=MINT, cutoff=NOW, creation_date=None, events=clean, prior_calls_by_channel={})
    spam_features = build_historical_features(mint=MINT, cutoff=NOW, creation_date=None, events=spam, prior_calls_by_channel={})
...
def test_outcome_requires_price_near_cutoff_without_saved_call_price() -> None:
    assert build_historical_outcome(
        cutoff=NOW, metrics=[metric(2, 1.0), metric(3, 2.0)], horizon_hours=72,
...
def feature(value: float, risk: float = 20) -> HistoricalFeatures:
    return HistoricalFeatures(
        cutoff=NOW, mint=MINT, x_mentions=1, x_authors=1, x_verified_ratio=0,
        x_suspicious_ratio=0, x_engagement=0, tg_mentions=1, tg_channels=1,
        tg_explicit_calls=1, historical_channels=1, channel_score=50,
        channel_win_rate=50, channel_rug_rate=0, copy_ratio=0, positive_sentiment=50,
        cross_platform_lag_minutes=1, early_minutes=5, x_score=value, tg_score=value,
        organic_score=80, manipulation_score=10, social_risk=risk, cross_score=80,
        early_score=80, core_social_score=value,
    )


def outcome(hit: bool, multiple: float) -> HistoricalOutcome:
    return HistoricalOutcome(
        baseline_price=1, final_price=1, peak_price=multiple, max_multiple=multiple,
        final_return_pct=0, max_drawdown_from_peak_pct=0,
        outcome="win" if hit else "loss", hit_20pct=multiple >= 1.2,
        hit_50pct=multiple >= 1.5, hit_2x=hit,
    )
...
def test_walk_forward_threshold_selected_on_train_then_applied_to_test() -> None:
    ...
    samples = [
        BacktestSample(
            mint=f"{MINT[:-2]}{index:02d}", cutoff=NOW + timedelta(days=index), channel_id=1,
            features=feature(value), outcome=outcome(hit, 2.2 if hit else 1.2),
        )
...
def test_high_social_risk_blocks_selection_even_with_high_score() -> None:
    samples = [
        ...
        BacktestSample(MINT[:-1] + "B", NOW + timedelta(days=2), 1, feature(50), outcome(False, 1.1)),
        BacktestSample(MINT[:-1] + "C", NOW + timedelta(days=3), 1, feature(45), outcome(False, 1.1)),
        BacktestSample(MINT[:-1] + "D", NOW + timedelta(days=4), 1, feature(42), outcome(False, 1.1)),
        BacktestSample(MINT[:-1] + "E", NOW + timedelta(days=5), 1, feature(40), outcome(False, 1.1)),
    ]
