...
def test_progress_stages_are_derived_from_scanner_output() -> None:
    assert _progress_for_line(
        "TeraGram: discovering CSV/Parquet relations"
    ) == (10, "discovering")


    assert _progress_for_line(
        "TeraGram: building disk-backed candidate and message indexes"
    ) == (45, "prefilter")


    assert _progress_for_line(
        "TeraGram: applying exact POTAPoff Telegram parser and scoring"
    ) == (70, "scoring")
