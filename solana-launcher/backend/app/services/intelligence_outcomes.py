...
async def evaluate_snapshot_horizon(
    ...
    *,
    ...
    result = await persist_outcome_values(
        ...
        snapshot=snapshot,
        horizon_hours=horizon_hours,
        baseline_price_usd=float(baseline_row.price_usd or 0.0),
        max_price_usd=max(float(row.price_usd or 0.0) for row in outcome_rows),
        min_price_usd=min(float(row.price_usd or 0.0) for row in outcome_rows),
        final_price_usd=float(final_candidates[-1].price_usd or 0.0),
