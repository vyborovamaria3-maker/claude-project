...
def historical_caller_reputation(rows: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    rugs = sum(
        final_to_peak is not None and final_to_peak <= 0.20 for _, _, final_to_peak in evaluated
    )
    ...
    score = (
        _clamp(outcome_score * 0.60 + originality * 0.25 + experience * 0.15) if evaluated else None
    )
    return {
    ...
    for level in LEVELS:
            ...
            if samples:
                ...
                windows[label] = {
                    ...
                    "positive_close_rate": round(
                        sum(value > 1 for value in closes) / len(closes), 4
                    ),
        ...
        result[level] = {
            ...
            "median_independence": round(
                median(
                    [
                        float(row["independence_score"])
                        for row in level_rows
                        if row.get("independence_score") is not None
                    ]
                ),
                2,
            )
            if any(row.get("independence_score") is not None for row in level_rows)
            else None,
...
def _independence_from_layers(
    ...
    independent_layers = sum(
        value is not None and value >= 55 for value in (tg_score, x_score, chain_score)
    )
    return (
    ...
    baseline = (
        await session.execute(
            ...
            .where(
                ...
                >= start - timedelta(minutes=BACKTEST_BASELINE_MAX_AGE_MINUTES),
            )
...
async def _x_sources_until(
    ...
    *,
    ...
    return len(
        {str(event.source_handle or "").strip().lower() for event in events if event.source_handle}
    )
    ...
    *,
    ...
    return {
        ...
        "limitations": [
            ...
            "Missing or stale TokenMetric baseline history excludes a signal "
            "instead of treating its return as zero.",
            "Legacy path limits calls before mint de-duplication; "
            "production CLI uses intelligence_backtest_v2.",
        ],
