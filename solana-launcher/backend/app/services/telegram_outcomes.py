...
def build_outcome_windows(
    ...
    *,
    ...
    for label, minutes in OUTCOME_WINDOWS_MINUTES:
        ...
        caps = [
            value for value in (_metric_cap(metric) for metric in eligible) if value is not None
        ]
        prices = [
            value for value in (_metric_price(metric) for metric in eligible) if value is not None
        ]
        ...
        result[label] = {
            ...
            "return_pct": _round(
                (close_multiple - 1) * 100 if close_multiple is not None else None, 3
            ),
            "final_to_peak": _round(final_to_peak),
            "drawdown_from_peak_pct": _round(
                (1 - final_to_peak) * 100 if final_to_peak is not None else None, 3
            ),
        }
