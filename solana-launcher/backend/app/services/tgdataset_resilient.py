...
def _is_retryable_stream_error(exc: BaseException) -> bool:
    if isinstance(exc, HTTPError):
        return int(exc.code) in _RETRYABLE_HTTP_STATUS
...
def scan_archives_resilient(
    *,
    ...
    for archive_name in selected:
        ...
        if (
            resume
            and archive_name in completed_archives
        ...
        checkpoint = (
            _read_checkpoint(checkpoint_path, archive_name)
            if resume
            else _read_checkpoint(Path("__missing__"), archive_name)
        )
        ...
        if progress is not None:
            ...
            suffix = (
                f"; resume after {resume_members} completed JSON files" if resume_members else ""
            )
            progress(
                f"[{archive_name}] starting "
                f"{'local' if source_path else 'Zenodo stream'} scan{suffix}"
            )
        ...
        while True:
            try:
                ...
                if source_path is not None or attempt >= retries or not retryable:
                    raise
                attempt += 1
        ...
        stats_payload["read_timeout_seconds"] = (
            read_timeout_seconds if source_path is None else None
        )
        ...
        if stopped_early:
            ...
            if progress is not None:
                progress(
                    f"[{archive_name}] development limit reached; "
                    "partial candidates promoted for output generation"
                )
            continue
