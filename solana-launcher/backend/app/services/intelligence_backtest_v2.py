...
async def run_intelligence_backtest_v2(
    ...
    *,
    ...
    for mint, signal_time in first_signals:
        ...
        prior_evaluated = sum(int(profile.get("evaluated") or 0) for profile in caller_profiles.values())
        signals.append(
            {
                "mint": mint,
    ...
    return {
        ...
        "limitations": [
            ...
            f"Missing TokenMetric history or a decision baseline older than {BACKTEST_BASELINE_MAX_AGE_MINUTES} minutes excludes a signal instead of treating its return as zero.",
        ],
