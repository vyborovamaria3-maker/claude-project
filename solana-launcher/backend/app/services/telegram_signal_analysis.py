...
def _is_forwarded_payload(payload: Any) -> bool:
    if not isinstance(payload, dict):
        return False
    return (
        bool(_forwarded_from(payload))
        or _flag(payload.get("is_forwarded"))
        or _flag(payload.get("forwarded"))
    )
...
def analyze_coordination_events(events: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    burst_5m = sum(1 for row in rows if (row["occurred_at"] - first_time).total_seconds() <= 5 * 60)
    ...
    spread_minutes = (rows[-1]["occurred_at"] - rows[0]["occurred_at"]).total_seconds() / 60.0
    return {
        "sources": total,
...
def build_caller_reputation(
    ...
    *,
    ...
    for username, caller_call_rows in by_caller.items():
        ...
        rois = [
            float(row["roi_multiple"]) for row in evaluated if row.get("roi_multiple") is not None
        ]
        ...
        result.append(
            {
                ...
                "evaluated": len(evaluated),
                "wins": wins,
                # Unknown outcome coverage stays null instead of looking like a 0% record.
                ...
                "temporal_outcome_score": (
                    round(temporal_outcome_score, 1) if temporal_outcome_score is not None else None
                ),
        ...
        rows.append(
            {
                ...
                "forwarded_from": (
                    forwarded_source or ("__forwarded__" if _is_forwarded_payload(raw) else None)
                ),
                "meta": call.meta or {},
            }
        )
