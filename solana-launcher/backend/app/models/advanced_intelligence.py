...
class CampaignFingerprint(Base):
    ...
    __table_args__ = (
        UniqueConstraint("snapshot_id", name="uq_campaign_fingerprints_snapshot"),
        Index("ix_campaign_fingerprints_mint_created", "mint_address", "created_at"),
...
class IntelligenceHypothesisState(Base):
    ...
    __table_args__ = (
        UniqueConstraint("hypothesis_key", name="uq_intelligence_hypothesis_key"),
        Index("ix_intelligence_hypothesis_status_updated", "status", "updated_at"),
        Index("ix_intelligence_hypothesis_mint", "mint_address", "updated_at"),
    )
...
class IntelligenceOutcome(Base):
    ...
    __table_args__ = (
        UniqueConstraint("snapshot_id", "horizon_hours", name="uq_intelligence_outcome_snapshot_horizon"),
        Index("ix_intelligence_outcomes_mint_horizon", "mint_address", "horizon_hours"),
    )
    ...
    outcome_label: Mapped[str] = mapped_column(String(64), nullable=False, default="unknown")
    payload: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class IntelligenceCalibrationStat(Base):
    ...
    __table_args__ = (
        UniqueConstraint("model_version", "signal_type", "bucket", name="uq_intelligence_calibration_bucket"),
        Index("ix_intelligence_calibration_signal", "signal_type", "updated_at"),
    )
    ...
    predicted_confidence_sum: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
...
class IntelligenceNarrativeMemory(Base):
    ...
    __table_args__ = (
        UniqueConstraint("narrative_key", name="uq_intelligence_narrative_key"),
