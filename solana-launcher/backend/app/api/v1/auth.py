...
async def phantom_nonce(
    ...
    expires_at = datetime.now(UTC) + timedelta(minutes=settings.phantom_nonce_ttl_minutes)
    user.nonce = nonce
...
async def phantom_verify(
    ...
    valid_nonce = (
        ...
        and _as_utc(user.nonce_expires_at) >= datetime.now(UTC)
    )
    try:
        if not valid_nonce:
            raise ValueError("Invalid nonce")
        if user is None or user.nonce_expires_at is None:
            raise ValueError("Invalid nonce")
        message = build_phantom_message(
            app_name=settings.app_name,
            wallet_address=payload.wallet_address,
...
async def login_password(
    ...
    token_result = await issue_token_for_user(session, user, settings)
