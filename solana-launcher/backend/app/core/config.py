...
class Settings(BaseSettings):
    ...
    def validate_production_security(self) -> "Settings":
        ...
        if not password or password == "ChangeMe123!" or len(password) < 16:
            raise ValueError(
                "ADMIN_PASSWORD must be explicitly configured with at least "
                "16 characters in production"
            )
        if (
            not session_secret
            or session_secret == "admin-session-secret"
            or len(session_secret) < 32
        ):
            raise ValueError(
                "ADMIN_SESSION_SECRET must be explicitly configured with at least "
                "32 characters in production"
            )
        if not backend_api_key or len(backend_api_key) < 32:
            raise ValueError(
                "BACKEND_API_KEY must be explicitly configured with at least "
                "32 characters in production"
            )
