...
class Settings(BaseSettings):
    ...
    frontend_internal_url: str = Field(
        default="http://frontend:3000", alias="FRONTEND_INTERNAL_URL"
    )
    ...
    telegram_session_path: str = Field(
        default="data/telegram-intelligence", alias="TG_SESSION_PATH"
    )
    ...
    telegram_evaluate_interval_seconds: int = Field(
        default=300, ge=60, alias="TG_EVALUATE_INTERVAL_SECONDS"
    )
    ...
    telegram_public_web_history_limit: int = Field(
        default=100, ge=1, le=500, alias="TG_PUBLIC_WEB_HISTORY_LIMIT"
    )
    telegram_public_web_timeout_seconds: float = Field(
        default=12.0, ge=2.0, le=60.0, alias="TG_PUBLIC_WEB_TIMEOUT_SECONDS"
    )
    telegram_public_web_discovery_enabled: bool = Field(
        default=True, alias="TG_PUBLIC_WEB_DISCOVERY_ENABLED"
    )
    telegram_public_web_discovery_depth: int = Field(
        default=2, ge=0, le=3, alias="TG_PUBLIC_WEB_DISCOVERY_DEPTH"
    )
    telegram_public_web_discovery_entity_limit: int = Field(
        default=25, ge=1, le=500, alias="TG_PUBLIC_WEB_DISCOVERY_ENTITY_LIMIT"
    )
    telegram_public_web_discovery_history_limit: int = Field(
        default=40, ge=10, le=200, alias="TG_PUBLIC_WEB_DISCOVERY_HISTORY_LIMIT"
    )
    telegram_public_web_relevance_min_score: float = Field(
        default=35.0, ge=0.0, le=100.0, alias="TG_PUBLIC_WEB_RELEVANCE_MIN_SCORE"
    )
    telegram_public_web_seed_database: str = Field(
        default="data/tgdataset/telegram_seed_database.json", alias="TG_PUBLIC_WEB_SEED_DATABASE"
    )
    telegram_public_web_seed_database_limit: int = Field(
        default=12, ge=0, le=100, alias="TG_PUBLIC_WEB_SEED_DATABASE_LIMIT"
    )
    telegram_public_web_refresh_enabled: bool = Field(
        default=True, alias="TG_PUBLIC_WEB_REFRESH_ENABLED"
    )
    telegram_public_web_refresh_tick_seconds: int = Field(
        default=60, ge=30, le=3600, alias="TG_PUBLIC_WEB_REFRESH_TICK_SECONDS"
    )
    telegram_public_web_refresh_batch_size: int = Field(
        default=25, ge=1, le=200, alias="TG_PUBLIC_WEB_REFRESH_BATCH_SIZE"
    )
    telegram_public_web_refresh_strong_seconds: int = Field(
        default=600, ge=60, alias="TG_PUBLIC_WEB_REFRESH_STRONG_SECONDS"
    )
    telegram_public_web_refresh_normal_seconds: int = Field(
        default=1800, ge=60, alias="TG_PUBLIC_WEB_REFRESH_NORMAL_SECONDS"
    )
    telegram_public_web_refresh_rejected_seconds: int = Field(
        default=21600, ge=300, alias="TG_PUBLIC_WEB_REFRESH_REJECTED_SECONDS"
    )
    telegram_public_web_refresh_unavailable_seconds: int = Field(
        default=3600, ge=300, alias="TG_PUBLIC_WEB_REFRESH_UNAVAILABLE_SECONDS"
    )
    ...
    collector_metric_refresh_seconds: int = Field(
        default=3600, alias="COLLECTOR_METRIC_REFRESH_SECONDS"
    )
    collector_insider_refresh_seconds: int = Field(
        default=86400, alias="COLLECTOR_INSIDER_REFRESH_SECONDS"
    )
    ...
    def validate_secret_key(cls, v: str) -> str:
        if len(v) < 32:
            raise ValueError(
                "SECRET_KEY must be at least 32 characters long. Generate a strong key "
        ...
        if any(pattern in v_lower for pattern in weak_patterns):
            raise ValueError(
                "SECRET_KEY contains weak pattern. Do not use default or predictable secrets."
            )
        return v
    ...
    def validate_runtime_security(self) -> "Settings":
        ...
        if not password or len(password) < 16:
            raise ValueError(
                "ADMIN_PASSWORD must be explicitly configured with at least 16 characters "
                "in production"
            )
        if not session_secret or len(session_secret) < 32:
            raise ValueError(
                "ADMIN_SESSION_SECRET must be explicitly configured with at least 32 "
        ...
        if not backend_api_key or len(backend_api_key) < 32:
            raise ValueError(
                "BACKEND_API_KEY must be explicitly configured with at least 32 characters "
                "in production"
            )
        if self.require_subscription_internal_key and (
            ...
            raise ValueError(
                "SUBSCRIPTION_INTERNAL_KEY must be explicitly configured with at least 32 "
                "characters for the API service"
            )
        if subscription_internal_key and len(subscription_internal_key) < 32:
            raise ValueError(
                "SUBSCRIPTION_INTERNAL_KEY must be at least 32 characters when configured"
            )
        if self.require_subscription_admin_key and (
            not subscription_admin_key or len(subscription_admin_key) < 32
        ):
            raise ValueError(
                "SUBSCRIPTION_ADMIN_KEY must be explicitly configured with at least 32 "
                "characters for the API service"
            )
        if subscription_admin_key and len(subscription_admin_key) < 32:
            raise ValueError(
                "SUBSCRIPTION_ADMIN_KEY must be at least 32 characters when configured"
            )
        ...
        if len(set(internal_keys)) != len(internal_keys):
            raise ValueError(
                "BACKEND_API_KEY, SUBSCRIPTION_INTERNAL_KEY and SUBSCRIPTION_ADMIN_KEY "
                "must be different credentials"
            )
        if "*" in self.cors_origins:
            raise ValueError("Wildcard CORS origins are not allowed in production")
        if len(encryption_key) < 32:
            raise ValueError(
                "SUBSCRIPTION_PASSWORD_ENCRYPTION_KEY must be explicitly configured with "
                "at least 32 characters in production"
            )
        if encryption_key == self.secret_key:
            raise ValueError(
                "SUBSCRIPTION_PASSWORD_ENCRYPTION_KEY must be different from SECRET_KEY"
            )
        return self
