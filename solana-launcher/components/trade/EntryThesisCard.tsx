...
function ListBlock({
  ...
  return (
    <div className={`rounded-xl border border-bg-border bg-bg-card p-3.5 ${className}`}>
      <div className="flex items-center gap-2 text-[9px] font-semibold uppercase tracking-wider text-content-faint">
        {icon}
        {title}
      </div>
      ...
      <div className="p-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="max-w-4xl">
            ...
            <h2 className="mt-2 text-base font-semibold leading-6 text-content">
              {thesis.headline}
            </h2>
          ...
          <div className={`shrink-0 rounded-xl border px-4 py-3 ${actionClass(thesis.action)}`}>
            ...
            <div className="mt-1 font-mono text-[10px] opacity-80">
              уверенность {Math.round(thesis.confidence)}%
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-xl border border-primary/20 bg-primary/5 p-3.5">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className={`text-[11px] font-semibold ${priceClass(thesis.priceState)}`}>
              {priceStateLabel(thesis.priceState)}
            </div>
          </div>
          <p className="mt-2 text-[12px] leading-5 text-content">
            {thesis.priceExplanation}
          </p>
        </div>
          ...
          <div className="mt-3 rounded-xl border border-neon-purple/25 bg-neon-purple/5 p-3.5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="text-[8px] text-content-faint">
                {thesis.qwenAgreement === "agree"
                  ? "совпадает с моделью"
                  : thesis.qwenAgreement === "partial"
                    ? "частично совпадает"
                    : thesis.qwenAgreement === "disagree"
                      ? "есть расхождение"
                      : "дополнительная интерпретация"}
              </span>
            </div>
            <p className="mt-2 text-[11px] leading-5 text-content-muted">
              {thesis.qwenView}
            </p>
          </div>
        )}
        ...
        <div className="mt-3 grid gap-3 xl:grid-cols-2">
          <ListBlock
            title="Почему вход рассматривается сейчас"
            rows={thesis.whyNow}
            icon={<CheckCircle2 className="h-3.5 w-3.5 text-success" />}
          />
          <ListBlock
            title="Что уже могло быть заложено в цену"
            rows={thesis.alreadyPricedIn}
            icon={<Clock3 className="h-3.5 w-3.5 text-warning" />}
          />
        </div>
        ...
        <div className="mt-3 grid gap-3 xl:grid-cols-2">
          <ListBlock
            ...
            rows={thesis.confirmationNeeded}
            icon={<AlertTriangle className="h-3.5 w-3.5 text-warning" />}
          />
          <ListBlock
            title="Что сломает входной тезис"
            rows={thesis.invalidation}
            icon={<ShieldAlert className="h-3.5 w-3.5 text-danger" />}
          />
        </div>

        <p className="mt-3 text-[9px] leading-4 text-content-faint">
          Оценка сравнивает текущую цену с доступным social/on-chain/market сигналом и не является оценкой фундаментальной стоимости или персональной рекомендацией.
        </p>
      </div>

      <details
        data-collapse="entry-thesis-technical"
