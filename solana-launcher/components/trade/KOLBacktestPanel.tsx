...
type BacktestResponse = {
  status?: string;
  coverage?: {
    joinedEventRows?: number;
    uniqueEvents?: number;
    eventsWithoutTokenRecord?: number;
...
export default function KOLBacktestPanel() {
  ...
  return (
    <section className={`${siteDesign.page.compactContainerClassName} !pt-0`} data-tag="trade.kols_backtest">
      <div className="surface-panel overflow-hidden rounded-2xl border border-bg-border">
        <div className="flex flex-col gap-3 border-b border-bg-border px-4 py-4 sm:px-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-3xl text-[11px] leading-5 text-content-muted">
              Историческая проверка accumulation/distribution по granular KOL swap-events. Сигнал использует только события,
              уже произошедшие к trigger; результат измеряется по независимым будущим token price metrics.
            </p>
          </div>
        ...
        <div className="grid gap-3 border-b border-bg-border p-4 sm:grid-cols-2 sm:p-5 xl:grid-cols-[.65fr_.65fr_.75fr_1.2fr_auto] xl:items-end">
          <SelectControl label="Lookback" value={lookbackDays} onChange={setLookbackDays} options={[[30, "30 days"], [60, "60 days"], [90, "90 days"], [180, "180 days"], [365, "365 days"]]} />
          <SelectControl label="Min KOLs / 1h" value={minKols} onChange={setMinKols} options={[[2, "2+"], [3, "3+"], [4, "4+"], [5, "5+"]]} />
          <SelectControl label="Confidence" value={minConfidence} onChange={setMinConfidence} options={[[50, "50+"], [70, "70+"], [90, "90+"]]} />
          ...
          <label className="flex min-h-[42px] items-center gap-2 rounded-xl border border-bg-border bg-bg-elevated px-3 text-xs text-content-muted">
            ...
            <span className="min-w-0">
              ...
              <span className="block text-[9px] leading-4 text-content-faint">ignore events before first attribution timestamp</span>
            </span>
          </label>

          <button type="button" onClick={() => void runBacktest()} disabled={loading} className={`${siteDesign.controls.primaryActionClassName} w-full xl:w-auto`}>
        ...
        {!data && !error ? (
          <div className="px-4 py-6 text-center text-xs text-content-muted sm:px-5">
            Запусти backtest после того, как в локальной базе накопились `kol_trade_events` и `token_metrics`.
          </div>
        ) : null}
        ...
        {data?.status === "ok" ? (
          <div className="space-y-5 p-4 sm:p-5">
              ...
              {(["1h", "6h", "24h"] as const).map((key) => {
                ...
                return (
                  <div key={key} className="rounded-xl border border-bg-border bg-bg-elevated/40 p-3.5">
                    ...
                    <div className="mt-2 text-2xl font-semibold text-content">{stats?.winRate == null ? "—" : `${stats.winRate.toFixed(1)}%`}</div>
                    <div className="mt-0.5 text-[10px] text-content-muted">directional win rate after 50 bps cost</div>
                    <div className="mt-3 grid grid-cols-2 gap-2 text-[10px]">
                      <div><div className="text-content-faint">Median</div><div className="mt-0.5 font-mono text-content">{percent(stats?.medianDirectionalReturnPct)}</div></div>
                      <div><div className="text-content-faint">Average</div><div className="mt-0.5 font-mono text-content">{percent(stats?.avgDirectionalReturnPct)}</div></div>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              <CoverageCard label="Unique events" value={data.coverage?.uniqueEvents ?? 0} />
              <CoverageCard label="Unmapped events" value={data.coverage?.eventsWithoutTokenRecord ?? 0} />
            ...
            {signals.length ? (
              <div className="overflow-hidden rounded-xl border border-bg-border">
                ...
                <div className="divide-y divide-bg-border">
                  {signals.slice(0, 20).map((signal) => {
                    ...
                    const accumulation = signal.signalType === "accumulation";
                    return (
                      <div key={`${signal.signalType}:${signal.mint}:${signal.signalAt}`} className="grid gap-2 px-3 py-3 text-xs md:grid-cols-[1fr_.65fr_.65fr_.45fr] md:items-center">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[9px] font-semibold uppercase ${accumulation ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-300" : "border-danger/30 bg-danger/10 text-danger"}`}>
                              {accumulation ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
                              {signal.signalType}
                            </span>
                        ...
                        <div><div className="font-semibold text-content">{signal.kolCount} KOLs</div><div className="mt-0.5 truncate text-[9px] text-content-muted">{signal.handles.slice(0, 4).map((handle) => `@${handle}`).join(" · ")}</div></div>
                        <div><div className="text-[9px] text-content-faint">24h directional</div><div className={`mt-0.5 font-mono font-semibold ${outcome && outcome.netDirectionalReturnPct > 0 ? "text-emerald-300" : outcome && outcome.netDirectionalReturnPct < 0 ? "text-danger" : "text-content-muted"}`}>{outcome ? percent(outcome.netDirectionalReturnPct) : "—"}</div></div>
                        <div className="text-left text-[10px] text-content-muted md:text-right">{formattedDate(signal.signalAt)}</div>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="rounded-xl border border-bg-border p-4 text-center text-xs text-content-muted">При выбранных параметрах исторические сигналы не сформировались.</div>
            )}
          </div>
        ) : null}
      </div>
...
function SelectControl({ label, value, onChange, options }: { label: string; value: number; onChange: (value: number) => void; options: Array<[number, string]> }) {
  return (
    <label className="block min-w-0">
      <span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-wider text-content-faint">{label}</span>
      <select value={value} onChange={(event) => onChange(Number(event.target.value))} className={siteDesign.controls.inputClassName}>
