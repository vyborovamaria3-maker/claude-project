...
type Props = {
  onImportTargets?: (usernames: string[]) => void | Promise<void>;
  importLimit?: number;
  className?: string;
};
...
export default function TeraGramInviteSource({
  ...
  const targets = useMemo(() => uniqueInviteTargets(channels), [channels]);

  async function importTargets() {
    if (!onImportTargets || targets.length === 0) return;
    setImporting(true);
    setError(null);
    try {
      await onImportTargets(targets);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Не удалось добавить каналы в TG Invite");
    } finally {
      setImporting(false);
    }
  }
    ...
    >
      ...
      <div className="relative space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="text-base font-bold text-content">База каналов / TeraGram</h3>
                <span
              ...
              <p className="mt-1 max-w-2xl text-sm leading-5 text-content-muted">
                Источник для TG Invite: сначала TeraGram отбирает crypto / Solana / memecoin каналы,
                затем выбранные username попадают в обычную очередь инвайтинга.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 gap-2 lg:grid-cols-5">
          <Metric label="Проверено чатов" value={status ? compactNumber.format(status.chats_total) : "—"} />
          <Metric label="Кандидаты" value={status ? fullNumber.format(status.candidate_channels) : "—"} />
          <Metric label="Seed для Invite" value={status ? fullNumber.format(status.seed_channels) : "—"} />
        ...
        {ready ? (
          <div className="rounded-2xl border border-bg-border bg-bg-soft/60 p-3 sm:p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <button
                type="button"
                onClick={() => void importTargets()}
                disabled={!onImportTargets || importing || loading || targets.length === 0}
                title={!onImportTargets ? "Подключите callback очереди TG Invite" : undefined}
              ...
              >
                ...
                {onImportTargets ? `Добавить ${targets.length} в TG Invite` : "Ожидает TG Invite"}
              </button>
            </div>
            ...
            {channels.length > 0 && (
              <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
                {channels.slice(0, 6).map((channel) => (
                  <div key={channel.username} className="rounded-xl border border-bg-border bg-bg-card px-3 py-2">
                    <div className="flex items-center justify-between gap-2">
                      <span className="truncate text-sm font-semibold text-content">@{channel.username}</span>
                      <span className="text-xs font-bold text-primary">{channel.seed_score.toFixed(1)}</span>
                    </div>
          ...
          <div className="rounded-2xl border border-dashed border-bg-border bg-bg-soft/40 p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-sm font-semibold text-content">Сначала нужен один успешный TeraGram scan</div>
                <p className="mt-1 text-xs leading-5 text-content-muted">
                  UI специально не запускает тяжёлый многотерабайтный job. После CLI/job scan здесь
                  автоматически появятся найденные каналы и кнопка импорта в TG Invite.
                </p>
              </div>
            </div>
          </div>
        ...
        {detailsOpen && (
          <div className="grid gap-2 text-xs text-content-muted sm:grid-cols-2 xl:grid-cols-4">
            ...
            <Detail label="Public discovery" value={status?.active_for_public_discovery ? "TeraGram active" : "legacy fallback"} />
            <Detail label="Zenodo preview" value={status?.source?.preview_record_id ? String(status.source.preview_record_id) : "18262126"} />
          </div>
        )}
      </div>
    </section>
