...
import { AlertTriangle, Shield, TrendingUp, Clock, RotateCcw } from "lucide-react";
...
// data-tag: token_launch.new.step4.bundle
export default function BundleReviewStep({ data, update, next, prev }: StepProps) {
  ...
  return (
    <div data-tag="token_launch.new.step4.bundle" className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="glass p-4 space-y-1">
            ...
            <p className="text-xs text-white/40 leading-relaxed mt-2">
              Derived from selected launch wallets on Step 3. Edit wallet amounts there to change the total bundle buy.
            </p>
          </div>
          {/* Dev Buy */}
          <div className="glass p-4 space-y-1">
            ...
            <p className="text-xs text-white/40 leading-relaxed mt-2">
              Derived from the dev wallet amount on Step 3. Set the dev wallet amount there to control the CREATE+BUY size.
            </p>
            <p className="text-xs text-white/40 leading-relaxed">
              Dev buy is 0 SOL — token will be created without an initial buy. Dev wallet still needs ~0.048 SOL for creation fees.
            </p>
          </div>
        </div>
        ...
        <div className="glass p-4 space-y-2">
          ...
          <p className="text-xs text-white/50">
            Derived from selected launch wallets on Step 3. Edit wallet amounts{" "}
            <span className="text-neon-green">there</span> to change the total bundle buy.
          </p>
        ...
        <div className="flex items-start gap-3 p-4 rounded-xl border border-yellow-500/30 bg-yellow-500/10">
          ...
          <div>
            ...
            <p className="text-xs text-yellow-400/70 mt-0.5">
              Go back to Step 3 and use "Fund" button to distribute SOL from your Phantom wallet to bundle wallets
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4 pt-2 border-t border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {JITO_TIERS.map((tier) => (
                <button
                  ...
                  className={["text-left w-full p-3 rounded-xl border transition", jitoTier === tier.id ? "border-neon-green bg-neon-green/10" : "border-bg-border bg-bg-card/40 hover:border-white/20"].join(" ")}
                >
            ...
            <p className="text-xs text-neon-green/70 leading-relaxed">
              Review still shows the backend-applied values, but the default launch preset here is now the cheapest valid one instead of an expensive hidden policy.
            </p>
          </div>
          {/* Landing Profile */}
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {LANDING_PROFILES.map((profile) => (
                <button
                  ...
                  className={["text-left w-full p-3 rounded-xl border transition", landingProfile === profile.id ? "border-neon-green bg-neon-green/10" : "border-bg-border bg-bg-card/40 hover:border-white/20"].join(" ")}
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="glass p-4 space-y-2">
            ...
            <input
              ...
              value={priorityFee === 0 ? "" : priorityFee}
              placeholder="0"
              onChange={(e) => setPriorityFee(e.target.value === "" ? 0 : Math.max(0, Number(e.target.value)))}
              className="w-full bg-bg-soft/60 border border-bg-border rounded-lg px-3 py-2 text-sm text-white font-mono focus:border-neon-green/50 outline-none"
            />
      ...
      <div className="space-y-4 pt-2 border-t border-white/10">
        ...
        <div className="flex items-center justify-between px-4 py-3 rounded-xl border border-bg-border bg-bg-card/40">
          ...
          <button
            ...
            className={["w-10 h-5 rounded-full relative transition", pumpFunCashback ? "bg-neon-green" : "bg-bg-border"].join(" ")}
          >
        ...
        <div className="grid grid-cols-4 bg-bg-card/40 border border-bg-border rounded-lg p-1">
          {(["anti_sniper", "smart_sell", "auto_tp", "retry"] as AdvancedTab[]).map((tab) => (
            <button
              ...
              className={["py-2 text-xs rounded-md transition", advancedTab === tab ? "bg-bg-soft text-white font-semibold" : "text-white/50 hover:text-white"].join(" ")}
            >
            ...
            {antiSniperEnabled && (
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-white/40 leading-relaxed">Uses the same pump.fun / on-chain trade feed as your activity list. When foreign (non-your-wallet) BUY share crosses the threshold, Stop Buying logs only; Sell Half / Sell All run automatic sells from bundle wallets.</p>
                </div>
                <div className="space-y-2">
