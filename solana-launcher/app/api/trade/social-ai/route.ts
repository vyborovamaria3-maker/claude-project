...
type QwenResult = {
  ...
  entryAssessment?: Record<string, unknown>;
...
type AdvancedReport = Record<string, unknown> & {
  layers?: {
    ...
    dynamic_research_budget?: { recommended_research_rounds?: number; recommended_entity_budget?: number };
    research_planner?: { candidates?: Array<{ entity?: string; expected_information_gain?: number }> };
    [key: string]: unknown;
  };
};

type ResearchRun = Awaited<ReturnType<typeof runBoundedResearch>>;
type SnapshotSize = { features: number; nodes: number; edges: number; evidence: number };
...
function stableId(item: TimelineItem, index: number) {
  return `social-${createHash("sha1").update(`${item.source_handle || item.source_name || "unknown"}|${item.occurred_at || ""}|${item.text || ""}|${index}`).digest("hex").slice(0, 24)}`;
}
...
function timelineMessages(timeline: TimelineItem[]) {
  return timeline.slice(0, 120).map((item, index) => {
    ...
    const sentAt = item.occurred_at && Number.isFinite(Date.parse(item.occurred_at)) ? new Date(item.occurred_at).toISOString() : new Date().toISOString();
    return {
...
function snapshotMessages(snapshot: AnalysisSnapshot) {
  return snapshot.evidence.slice(0, QWEN_SNAPSHOT_LIMITS.evidence).map((entry) => {
    const isX = entry.platform === "x";
    const sentAt = entry.timestamp && Number.isFinite(Date.parse(entry.timestamp)) ? new Date(entry.timestamp).toISOString() : snapshot.createdAt;
    return {
...
function edgeScore(edge: GraphEdge, nodeById: Map<string, GraphNode>) {
  ...
  const onChainSignal = edge.type === "trades"
    ? (attributes.wash === true ? 120 : 0) + (attributes.smart === true ? 90 : 0) + Math.min(60, Math.log10(Math.max(1, Number(attributes.volumeSol || 0))) * 20)
    : 0;
  return (EDGE_PRIORITY[edge.type] || 0) + edge.confidence * 100 + Math.min(25, edge.evidenceIds.length * 5) + onChainSignal;
}

function compactGraphForQwen(graph: AnalysisSnapshot["graph"]): AnalysisSnapshot["graph"] {
  if (graph.nodes.length <= QWEN_SNAPSHOT_LIMITS.nodes && graph.edges.length <= QWEN_SNAPSHOT_LIMITS.edges) return graph;
  const nodeById = new Map(graph.nodes.map((node) => [node.id, node]));
  const selectedNodeIds = new Set(graph.nodes.filter((node) => node.type === "token").map((node) => node.id));
  const selectedEdges: GraphEdge[] = [];
  const rankedEdges = [...graph.edges].sort((left, right) => edgeScore(right, nodeById) - edgeScore(left, nodeById));
  ...
  const nodePriority: Record<GraphNode["type"], number> = { token: 100, tg_channel: 90, x_account: 85, wallet: 80, bundle: 75, url: 60 };
  const remainingNodes = graph.nodes.filter((node) => !selectedNodeIds.has(node.id)).sort((left, right) => (nodePriority[right.type] || 0) - (nodePriority[left.type] || 0));
  ...
  const edges = selectedEdges.filter((edge) => selectedNodeIds.has(edge.source) && selectedNodeIds.has(edge.target));
  return {
...
type QwenSnapshot = Omit<AnalysisSnapshot, "features" | "graph" | "evidence" | "rawSummary" | "provenance"> & {
  features: Array<Pick<AnalysisSnapshot["features"][number], "key" | "group" | "label" | "value" | "numericValue" | "confidence" | "observedAt" | "missing" | "note"> & { source: "derived" | "x" | "telegram" | "market" | "chain" }>;
  graph: AnalysisSnapshot["graph"];
  evidence: AnalysisSnapshot["evidence"];
  rawSummary: {
    xPosts: number; xRiskUniversePosts: number; telegramMessages: number; telegramMatchedBeforeLimit: number;
    trades: number; wallets: number; bundles: number; chainTruncated: boolean; marketAvailable: boolean; marketStale: boolean;
    originalFeatures: number; originalGraphNodes: number; originalGraphEdges: number; originalEvidence: number;
    qwenGraphNodes: number; qwenGraphEdges: number; qwenEvidence: number; qwenGraphCompacted: boolean;
  };
};

function qwenSnapshot(snapshot: AnalysisSnapshot): QwenSnapshot {
  const { provenance: _provenance, ...baseSnapshot } = snapshot as AnalysisSnapshot & { provenance?: unknown };
  ...
  const features = [...snapshot.features]
    .sort((left, right) => {
      const leftPriority = (left.key.startsWith("scores.") ? 2 : 0) + (left.key.startsWith("advanced.") ? 1 : 0);
      const rightPriority = (right.key.startsWith("scores.") ? 2 : 0) + (right.key.startsWith("advanced.") ? 1 : 0);
      return rightPriority - leftPriority || right.confidence - left.confidence;
    })
    .slice(0, QWEN_SNAPSHOT_LIMITS.features)
    .map((feature) => ({
      key: feature.key, group: feature.group, label: feature.label, value: feature.value, numericValue: feature.numericValue,
      source: ["derived", "x", "telegram", "market", "chain"].includes(feature.source) ? feature.source as "derived" | "x" | "telegram" | "market" | "chain" : "derived",
      confidence: feature.confidence, observedAt: feature.observedAt, missing: feature.missing, ...(feature.note ? { note: feature.note } : {}),
    }));
  return {
    ...baseSnapshot,
    ...
    rawSummary: {
      ...
      qwenGraphCompacted: graph.nodes.length !== originalSize.nodes || graph.edges.length !== originalSize.edges,
    },
  };
}
...
async function runQwen(payload: NonNullable<ReturnType<typeof aiPayload>>, timeoutMs: number): Promise<QwenEnvelope> {
...
async function buildAdvancedReport(snapshot: AnalysisSnapshot, result: QwenEnvelope, options: { persist?: boolean; enrich?: boolean; role?: "analyst" | "critic"; timeoutMs?: number } = {}): Promise<AdvancedReport | null> {
  if (!BACKEND_KEY) return null;
  try {
    const response = await fetch(`${BACKEND_BASE}/api/v1/social/intelligence/advanced/report`, {
      method: "POST",
      headers: { "content-type": "application/json", "X-Backend-API-Key": BACKEND_KEY },
      body: JSON.stringify({ snapshot, ai_result: result.result || null, persist: options.persist ?? true, enrich: options.enrich ?? true, role: options.role || "analyst" }),
      cache: "no-store",
...
async function persistMainMemory(snapshot: AnalysisSnapshot, analysisSnapshot: AnalysisSnapshot, result: QwenEnvelope) {
  if (!BACKEND_KEY) return { status: "disabled" };
  try {
    const response = await fetch(`${BACKEND_BASE}/api/v1/social/intelligence/memory`, {
      method: "POST",
      headers: { "content-type": "application/json", "X-Backend-API-Key": BACKEND_KEY },
      body: JSON.stringify({ snapshot, analysis_snapshot: analysisSnapshot, ai_result: result.result || null, provider: typeof result.provider === "string" ? result.provider : null, model: typeof result.model === "string" ? result.model : null, prompt_version: INTELLIGENCE_PROMPT_VERSION }),
      cache: "no-store",
...
async function persistCriticAudit(snapshotId: string, criticSnapshot: AnalysisSnapshot, priorConclusion: string, critic: QwenEnvelope) {
  if (!BACKEND_KEY) return { status: "disabled" };
  const advancedFeatures = criticSnapshot.features.filter((feature) => feature.key.startsWith("advanced."));
  try {
    const response = await fetch(`${BACKEND_BASE}/api/v1/social/intelligence/memory/audit`, {
      method: "POST",
      headers: { "content-type": "application/json", "X-Backend-API-Key": BACKEND_KEY },
      body: JSON.stringify({ snapshot_id: snapshotId, key: "critic_v1", payload: { prompt_version: INTELLIGENCE_PROMPT_VERSION, advanced_features: advancedFeatures, prior_conclusion: priorConclusion, result: critic.result || null, provider: typeof critic.provider === "string" ? critic.provider : null, model: typeof critic.model === "string" ? critic.model : null } }),
      cache: "no-store",
      signal: AbortSignal.timeout(4_000),
    });
...
function compactPriorConclusion(result: QwenEnvelope) {
  const value = {
    ...
    entryAssessment: result.result?.entryAssessment || null,
    sourceAssessments: result.result?.sourceAssessments || null,
...
function remainingBudget(startedAt: number) { return Math.max(0, REQUEST_BUDGET_MS - (Date.now() - startedAt)); }

function advancedFeature(snapshot: AnalysisSnapshot, layer: string, value: unknown): AnalysisSnapshot["features"][number] {
  const serialized = JSON.stringify(value ?? null).slice(0, 600);
  return buildProvenanceFeature({ key: `advanced.${layer}`, group: "Advanced Intelligence", label: layer.replaceAll("_", " "), value: serialized, numericValue: null, source: "derived", confidence: 0.85, observedAt: snapshot.createdAt, missing: false, note: "Deterministic/historical advanced-intelligence layer; verify evidence before causal claims." });
}

function snapshotWithAdvancedReport(snapshot: AnalysisSnapshot, report: AdvancedReport | null) {
  if (!report?.layers) return snapshot;
  const existing = new Set(snapshot.features.map((item) => item.key));
  const additions = Object.entries(report.layers).filter(([key]) => !existing.has(`advanced.${key}`)).map(([key, value]) => advancedFeature(snapshot, key, value)).slice(0, Math.max(0, 300 - snapshot.features.length));
  if (!additions.length) return snapshot;
  const features = [...snapshot.features, ...additions];
  return { ...snapshot, featureCount: features.length, features, provenance: rebuildProvenanceSnapshot({ ...snapshot, features }).provenance };
}

function plannerCandidates(report: AdvancedReport | null) {
  const rows = report?.layers?.research_planner?.candidates || [];
  return rows.filter((row) => row.entity).sort((left, right) => Number(right.expected_information_gain || 0) - Number(left.expected_information_gain || 0)).map((row) => String(row.entity));
}

export async function POST(req: NextRequest) {
  const startedAt = Date.now();
  let body: RequestBody;
  try { body = (await req.json()) as RequestBody; } catch { return NextResponse.json({ error: "invalid_json" }, { status: 400 }); }
  ...
  try { snapshot = validateSnapshot(body, mint); }
    ...
    if (error instanceof SnapshotLimitError) return NextResponse.json({ error: error.message, snapshotSize: error.size, limits: error.limits }, { status: 400 });
  ...
  try {
    ...
    if (analysisSnapshot && remainingBudget(startedAt) > 26_000) {
      ...
      if (candidates.length) {
        ...
        if (analysisSnapshot.features.length > before && remainingBudget(startedAt) > 18_000) {
          const payload = aiPayload(analysisSnapshot, timeline, mint, body);
          if (payload) {
            result = await runQwen(payload, Math.min(32_000, Math.max(10_000, remainingBudget(startedAt) - 12_000)));
            researchRound = 1;
          }
        }
      }
    ...
    for (let round = researchRound + 1; analysisSnapshot && round <= recommendedRounds && round <= MAX_DYNAMIC_RESEARCH_ROUNDS; round++) {
      ...
      const candidates = [...new Set([...qwenCandidates, ...deterministicCandidates])].filter((entity) => !researched.has(entity)).slice(0, 8);
      ...
      for (const entity of run.requested || []) researched.add(entity);
      analysisSnapshot = run.snapshot;
      if (analysisSnapshot.features.length <= before || remainingBudget(startedAt) < 16_000) break;
      const payload = aiPayload(analysisSnapshot, timeline, mint, body);
      if (!payload) break;
      result = await runQwen(payload, Math.min(24_000, Math.max(8_000, remainingBudget(startedAt) - 10_000)));
      researchRound = round;
    }
    ...
    if (analysisSnapshot && remainingBudget(startedAt) > 12_000) {
      ...
      if (advancedIntelligence?.layers?.dedicated_critic?.required && remainingBudget(startedAt) > 14_000) {
        ...
        if (criticPayload) {
          try {
            critic = await runQwen(criticPayload, Math.min(20_000, Math.max(8_000, remainingBudget(startedAt) - 4_000)));
            if (remainingBudget(startedAt) > 4_500) await buildAdvancedReport(criticSnapshot, critic, { persist: true, enrich: false, role: "critic", timeoutMs: 4_000 });
            if (snapshot && remainingBudget(startedAt) > 4_000) criticAudit = await persistCriticAudit(snapshot.snapshotId, criticSnapshot, priorConclusion, critic);
          } catch { critic = null; criticAudit = { status: "failed" }; }
        }
      }
    }

    const lastResearch = researchRuns.length ? researchRuns[researchRuns.length - 1] : null;
    ...
    return NextResponse.json({
      ...
      requestBudget: { maxMs: REQUEST_BUDGET_MS, elapsedMs: Date.now() - startedAt, remainingMs: remainingBudget(startedAt) },
      memory: { loaded: Boolean(initialMemory), stats: initialMemory?.stats || null, write: memoryWrite, criticAudit, researchRound, recommendedResearchRounds: recommendedRounds, researchedEntities: [...researched], tools: researchMeta(lastResearch) },
      advancedIntelligence,
      critic: critic ? { available: true, result: critic.result || null, provider: critic.provider || null, model: critic.model || null } : { available: false },
      ...result,
    }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    const rawStatus = (error as { status?: unknown }).status;
    const status = typeof rawStatus === "number" ? rawStatus : 503;
    return NextResponse.json({ error: error instanceof Error ? error.message : "qwen_unreachable", agent: "qwen", available: false }, { status: status === 401 || status === 403 ? status : 503 });
  }
}
