...
const intelligenceFeatureSchema = z.object({
  ...
  value: z.union([z.string().max(512), z.number(), z.null()]),
  numericValue: z.number().finite().optional().nullable(),
  source: z.enum(['derived', 'x', 'telegram', 'market', 'chain']),
  confidence,
...
const intelligenceEdgeSchema = z.object({
  id: z.string().min(1).max(160),
...
const intelligenceEvidenceSchema = z.object({
  id: z.string().min(1).max(160),
  platform: z.enum(['x', 'telegram']),
  source: z.string().min(1).max(512),
  text: z.string().max(40_000),
  timestamp: dateString.optional().nullable(),
...
export const intelligenceSnapshotSchema = z.object({
  ...
  graphVersion: z.string().min(1).max(80),
  mint: z.string().min(1).max(128),
  symbol: z.string().max(32).optional().nullable(),
  tokenName: z.string().max(128).optional().nullable(),
  createdAt: dateString,
  featureCount: z.number().int().min(0).max(1_000),
  missingFeatureCount: z.number().int().min(0).max(1_000),
  features: z.array(intelligenceFeatureSchema).max(300),
  graph: z.object({
    version: z.string().min(1).max(80),
    nodes: z.array(intelligenceNodeSchema).max(500),
    edges: z.array(intelligenceEdgeSchema).max(1_500),
    stats: z.object({
...
export const telegramAiResultSchema = z.object({
  ...
  featureAssessments: z.array(z.object({
    featureKey: z.string().min(1).max(160),
    assessment: z.enum(['supportive', 'neutral', 'concerning', 'insufficient_data']),
  ...
  finalIntelligence: z.object({
    ...
    manipulationAssessment: z.string().min(1).max(1_500),
    bullCase: z.string().min(1).max(1_500),
    bearCase: z.string().min(1).max(1_500),
